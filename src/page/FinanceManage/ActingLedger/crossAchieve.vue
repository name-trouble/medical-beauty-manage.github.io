    ...
    <div class="actingLedger selCommon">
        ...
        <el-form :inline="true" :model="formInline" class="demo-form-inline form_search" label-width="80px">
            ...
            <el-form-item label="关键字："  class="form_item_mt10 form_item_wh280">
                <el-input v-model="formInline.keywords" placeholder="客户/消费商/医护人员/项目" style="width: 200px;" v-on:keyup.enter.native="onSubmit"></el-input>
            </el-form-item>
            ...
            <el-form-item label="推荐医院:" class="form_item_mt10">
                <el-select v-model="formInline.RefHospitalCode"  style='width:200px;' @change="selHos">
         ...
         :summary-method="getSummaries"  show-summary>
            ...
            <el-table-column prop="hospitalName" label="执行医院" min-width="80"></el-table-column>
            <el-table-column prop="refHospitalName" label="推荐医院" min-width="80"></el-table-column>
            ...
            <el-table-column prop="orderAmount" label="订单金额" min-width="65" align="right">
                <template slot-scope="scope">
                   {{scope.row.orderAmount.toQFW()}}
                </template>
            </el-table-column>
            <el-table-column prop="execCount" label="执行次数" min-width="65" align="right"></el-table-column>
            <el-table-column prop="execAmount" label="当日执行" min-width="65" align="right"></el-table-column>
            <el-table-column prop="expectAmount" label="已执行" min-width="65" align="right">
                <template slot-scope="scope">
                   {{scope.row.expectAmount.toQFW()}}
                </template>
            </el-table-column>
            <el-table-column prop="unExpectAmount" label="未执行" min-width="65" align="right">
                <template slot-scope="scope">
                   {{scope.row.unExpectAmount.toQFW()}}
                </template>
            </el-table-column>
             <el-table-column prop="serviceAmount" label="附加费" min-width="65" align="right">
                 <template slot-scope="scope">
...
<script type="text/ecmascript-6">
    ...
    import {GetCrossHosPerformance,GetDropDownPermission,GetDeptAllByHospitalCode,GetSupplierBySupTypeCode} from "@/api/myData"
    export default {
        // name:"actingLedger",
        data () {
            return {
        ...
        computed: {
            ...
            tHeader(){
                return ['执行日期','订单日期', '客户', '消费商', '咨询师', '设计师', '设计师助理', '医生','护士', '项目类型', '项目明细','科室',
                    '订单金额','执行次数','当日执行','已执行','未执行','附加费','执行状态']
            },
            filterVal(){
                return ['execDate','orderDate', 'customerName', 'refrenceBranchName', 'marketConsultantName', 'designerName', 'designerAssistName', 'doctorName','nurseName', 'itemTypeName', 'projectName',
                    'deptName','orderAmount','execCount','execAmount','expectAmount','unExpectAmount','serviceAmount','commandState']
            },
            tableData1(){
                ...
                return arr.concat({
                    execDate:"合计",orderDate:'',customerName:'',refrenceBranchName:'',hospitalName:'',marketConsultantName:'',refHospitalName:'',projectName:"",deptName:'',orderAmount:'',execCount:'',
                    execAmount:this.sum.sum6,expectAmount:this.sum.expectAmount,unExpectAmount:this.sum.unExpectAmount,serviceAmount:this.sum.sum7,commandState:""
                })
            },
            name(){
                return '业绩明细'
        ...
        methods: {
            ...
            getSummaries(param) {
                return ["总合计",'','','','','','','','','',this.sum.sum6.toQFW(),
                this.sum.expectAmount.toQFW(),this.sum.unExpectAmount.toQFW(),this.sum.sum7.toQFW()];
            },
            dateSelect(val){
                if(val == undefined){
                    val = ""
            ...
            async GetCrossHosPerformance(params){
                let res =await GetCrossHosPerformance(params)
                if(res.status){
            ...
            async GetDropDownPermission(){
                try {
                    ...
                    let hosList = await GetSupplierBySupTypeCode({
                        SupTypeCode:'001'
                    })
                    ...
                    this.formInline.hospitalId = getCookie("hospitalCode")
            ...
            search(){
                ...
                this.GetCrossHosPerformance({
                    ...
                    typeId:1,
                     ...
                     RefHospitalCode:this.formInline.RefHospitalCode
