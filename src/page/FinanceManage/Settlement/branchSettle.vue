<template>
    <div class="settled selCommon">
        ...
        class="min_table smt" max-height="660" show-summary  >
            ...
            <el-table-column label="操作" prop="hospitalName" width="120">
                <template slot-scope="scope">
        ...
        <el-dialog title="详情" :visible.sync="dialogTableVisible" size="">
            ...
            class="min_table smt"  @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="fxCode" label="订单" min-width="90" align="center">
                    <template slot-scope="scope">
                        <el-button @click="getPD(scope.row)" type="text">{{scope.row.fxCode}}</el-button>
                    </template>
                </el-table-column>
                <el-table-column prop="hospitalName" label="销售公司" min-width="100"> </el-table-column>
                <el-table-column prop="toName" label="转到" min-width="150" > </el-table-column>
                <el-table-column prop="rate" label="比例" min-width="100" align="right">
                    <template slot-scope="scope">
...
<script type="text/ecmascript-6">
    ...
    import {GetCrossHospitalDetail,GetCompanyOut,GetDropDownPermission,ComOutSettle,GetCompanyOutDetail,GetSupplierBySupTypeCode} from "@/api/myData"
...
//            设置默认中国时间
        ...
        methods: {
            ...
            async GetCompanyOutDetail(params){
                let res = await GetCompanyOutDetail(params)
...
//            结算
            async ComOutSettle(params){
                try {
                    let res = await ComOutSettle(params)
                    if(res.status){
...
//            获取列表信息
            async GetCompanyOut(params){
                try {
                    this.loading = true
                    let res = await GetCompanyOut(params)
                    this.loading = false
                    if(res.status){
