                        ...
                        <div class="flex-item">
                            <el-form label-position="right" label-width="120px" :model="formData" ref="formAdd1">
                                <el-form-item label="订单日期：" prop="OrderDate" :rules="[ { required: true, message: '不能为空'}]" class=" form_item_mt0">
                                    <el-date-picker v-model="orderDate" @change="dateChange"  type="date" placeholder="选择日期" style="width:220px;" popper-class="data-picker">
                                    </el-date-picker>
                                </el-form-item>

                                <el-form-item label="规则类型：" prop="ConsumeTypeId" :rules="[{ required: true, message: '不能为空',trigger: 'blur'}]"  class=" form_item_mt0">
                                ...
                                <!--h('p', { attrs: { class: 'select_name'+item.Type, title: item.name } }, ['名字：' + item.name+item.SourcWayeCode]),-->
                                ...
                                <el-form-item label="客户：" prop="customerId" :rules="[ { required: true, message: '不能为空'}]" class=" form_item_mt0">
                                    <el-autocomplete v-model="customer" @change="cusChange($event)" :fetch-suggestions="queryCus" placeholder="请输入内容"
                                     :trigger-on-focus="false" @select="selectCus" style="width:220px;margin-right:5px">
                                        <template v-slot="{item}">
                                            <my-item-ReportMember :item="item"></my-item-ReportMember>
                                        </template>
                                    </el-autocomplete>
                                    <div>电话：{{mesShow.cusPhoneNumber}}，卡号：{{mesShow.cusCardNO}} <span style="color:red"> {{isFrozen}}</span></div>
                                </el-form-item>
                                ...
                                <el-form-item label="项目：" class=" form_item_mt0">
                                    <el-autocomplete v-model="project" :fetch-suggestions="queryPro" placeholder="请输入内容" style="width:220px;float:left;margin-right:5px"
                                      :trigger-on-focus="false" @select="selectPro">
                                        <template v-slot="{item}">
                                 ...
                                 <el-form-item label="疗程：" class=" form_item_mt0">
                        ...
                        <div class="flex-item">
                            <el-form label-position="right" label-width="120px" :model="formData" ref="formAdd2">
                                ...
                                <el-form-item label="积分：" class=" form_item_mt0">
                                    <span style="line-height:36px">医院积分：{{jifen}}，纹绣积分：{{wxjf}}，皮肤科积分：{{pfkjf}}，app积分：{{jifenOnline}}，至尊积分：{{vipJF}}</span>
                                </el-form-item>
                                <el-form-item label="推荐人：" prop="refrenceBranchCode" class=" form_item_mt0">
                                    <div v-show="formData.refrenceBranchCode!==''">{{ '(' + formData.refrenceBranchCode +')' + formData.refrenceBranchName }} <span v-if="mesShow.BranchTags">({{mesShow.BranchTags}})</span>， 电话：{{mesShow.refPhoneNumber}}</div>
                                ...
                                <el-form-item label="咨询师：" class=" form_item_mt0">
                                    <el-select v-model="consultCodeName" style="width:220px;">
                                        <el-option label="空" value=""></el-option>
                                ...
                                <el-form-item label="设计师：" class=" form_item_mt0" >
                                    <el-select v-model="formData.designer" style="width:220px;">
                                        <el-option label="空" value=""></el-option>
                ...
                <div class="flex-box" style="">
                    <div class="flex-item" style="padding-left:30px;flex:0 100%">
                        <el-table :data="attachList" border style="width:840px">
                            ...
                            <el-table-column prop="Course" label="总期数" min-width="60"></el-table-column>

                            <el-table-column prop="CurrentCourse" label="支付期数" min-width="60"></el-table-column>

                            <el-table-column prop="SubTotal" label="总额" min-width="60"></el-table-column>

                            <el-table-column prop="StageAmount" label="分期总额" min-width="60"></el-table-column>
                        ...
                        <el-form label-position="right" label-width="120px" :model="formData" ref="formAdd3">
                            ...
                            <!-- <el-form-item label="图片：">
                                ...
                                <div class="flex-box img-box">
                                    <el-upload class="upload-box" :accept="acceptImage" :action="uploadUrl" :on-change="imgChange"  list-type="picture-card" :file-list="fileList" :before-upload="beforeUpload">
...
<script type="text/ecmascript-6">
    ...
    import { AddProofOrder, GetUserAllByKeywords, GetGoodsByKeywords, GetBranchByCode,GetPoints,
    ...
    Vue.component('my-item-reportPro', {
     ...
     Vue.component('my-item-ReportMember', {
        functional: true,
        render: function(h, ctx) {
    ...
    export default {
        ...
        mounted() {
            console.log('添加报单', this.reservationId);
            this.orderDate = new Date()
            this.formData.OrderDate = new Date()
            if(this.customerMes){//客户详情、复诊页面的快速开单赋值操作
                this.customer = this.customerMes.MemberName
                this.formData.customerId = this.customerMes.Code
                this.quickReport(this.customerMes)
            }else{
                this.getConsume()
            }
        },
        ...
        methods: {
            ...
            async quickReport(customer){
                if(customer.hospitalCode){//复诊
                    this.formData.hospitalId = customer.hospitalCode
                    this.formData.hospitalName = customer.hospitalName
                    this.formData.designerAssist = customer.designerAssist?customer.designerAssist:''//复诊带来的设计师和设计师助理
                    this.formData.designer = customer.designer?customer.designer:''
                    this.filterDoctor(customer.hospitalCode)
                }
                let mes = await GetUserAllByKeywords({ keywords:customer.Code})
                this.selectCus(mes[0])
            },
            //点击获取 会员信息  Oject
            selectCus(item) {
                this.formData.BranchTags = item.BranchTags,
                this.formData.customerId = item.Code
                this.formData.customerName = item.MemberName
            ...
            async GetUserAccountByCode(params){
                ...
                this.pfkjf = res.SkinRemainAmount
                this.vipJF = res.VIPRemainPoints
            ...
            async filterDoctor(hospitalCode) {
                ...
                for (let item of this.serciceManList) {
                    ...
                    if(item.serTypeCode === "004"){//咨询师 不做医院限制
            ...
            async addData(valStr) {
                ...
                // if((this.formData.OrderDate+"").length>20){
                //     this.formData.OrderDate = this.orderDate.formatDate('yyyy-MM-dd hh:mm:ss')
                // }
                if (!this.formData.OrderDate) {
                    this.formData.OrderDate = new Date()
                }
                if (this.formData.OrderDate instanceof Date) {
                    this.formData.OrderDate = this.orderDate.formatDate('yyyy-MM-dd hh:mm:ss')
                }
