<template>
    <div>
        ...
        <el-table :data="tableData" border style="width: 100%;margin-top:15px;margin-bottom:10px;" max-height="560" class="min_table" v-loading="loading">
            ...
            <el-table-column prop="OrderType" label="规则类型" width="90">
                <template slot-scope="scope">
            ...
            <el-table-column prop="RefundAmount" label="退款金额" min-width="90" align="right">
                <template slot-scope="scope">
            ...
            <el-table-column prop="CommandState" label="操作" fixed="right" width="140">
                <template slot-scope="scope">
                    <el-button @click="command(scope.$index,'command')" type="primary" size="small" :disabled="scope.row.CommandState==2 || scope.row.IsPayOff==false||scope.row.ApproveState ==6||scope.row.ApproveState==10">执行</el-button>
                    <el-button @click="commandEdit(scope.$index,'operate')" type="primary" size="small" :disabled="scope.row.IsPayOff==false||scope.row.ApproveState ==6||scope.row.ApproveState==10">编辑</el-button>
                    <!-- 已付清未审核 -->
                    <!-- <el-button @click="command(scope.$index,'edit')" type="primary" size="small" :disabled="scope.row.IsPayOff==false||scope.row.ApproveState!=0">编辑</el-button> -->
                </template>
            </el-table-column>
        </el-table>
        ...
        <el-dialog ref="pay" title="执行" :visible.sync="isPopCmd" top="5%" @close="isPopCmd=false" size=""  :close-on-click-modal="false">
            <pop-cmd v-if="isPopCmd" @close='cmdClose' :fx-code="fxCode" :operate="operate" @photoShow='photoShow'></pop-cmd>
        </el-dialog>
        <el-dialog ref="pay" title="编辑" :visible.sync="isPopCmdE" top="5%" @close="isPopCmd=false" size=""  :close-on-click-modal="false">
            <pop-cmd v-if="isPopCmdE" @close='cmdClose' :fx-code="fxCode" :operate="operate" @photoShow='photoShow'></pop-cmd>
        </el-dialog>
        ...
        <el-dialog v-model="dialogPhoto" size="tiny">
            <div>
...
<script type="text/ecmascript-6">
    ...
    var _this
    export default {
        components: { reportSearch, PopCmd, PopRead,CustomerView  },
        data() {
            return {
                dialogPhoto:false,
                dialogImageUrl:"",
        ...
        methods: {
            ...
            cmdClose(data) {
                this.isPopCmd = false
                this.isPopCmdE = false
                if (data === 1) this.$refs.search.onSubmit()
            },

            commandEdit(index,operate) {
                this.operate = operate
                this.fxCode = this.tableData[index]["FxCode"]
                this.isPopCmdE = true
            },
