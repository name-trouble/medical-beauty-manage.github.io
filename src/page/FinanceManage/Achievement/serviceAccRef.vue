...
>
         ...
         <el-table  :data="tableData" border style="width: 100%;margin-top:20px" max-height="560" class="min_table" show-summary v-loading="loading"  :summary-method="getSummaries">
            ...
            <el-table-column prop="referrerName" label="服务商" min-width="80">
                <template slot-scope="scope">
                    <span v-if="scope.row.referrerName">{{scope.row.referrerName}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="refBranchName" label="消费商推荐" min-width="80"></el-table-column>
            ...
            <el-table-column prop="commissionAmount" label="提成金额" min-width="80" align="right">
                <template slot-scope="scope">
                    <span v-if="scope.row.commissionAmount == 0">0</span> 
                    <el-button v-else type="text" @click="view(scope.row)">{{scope.row.commissionAmount.toQFW()}}</el-button>
                </template>
            </el-table-column>
        ...
        <el-dialog title="详情" :visible.sync="dialogView"  top="5%"  size="" :close-on-click-modal="false">
            <span>总金额：{{totalAmount}}</span>  <span>结算金额：{{settleAmount}}</span>
             ...
             <!-- <el-button type="primary" style="margin-left:20px" @click="Dsettle">结算</el-button> -->
            <el-table  :data="detData" border style="margin-top:20px;width:600px" max-height="460" class="min_table"
                    @selection-change="handleSelectionChange">
            <el-table-column type="selection"   align="center" width="55"></el-table-column>
            <el-table-column prop="referrerName" label="推荐人" min-width="80"></el-table-column>
            <el-table-column prop="serviceName" label="服务商" min-width="80"></el-table-column>
            <el-table-column prop="refBranchName" label="消费商" min-width="80"></el-table-column>
                <el-table-column prop="commissionAmount" label="提成金额" min-width="80" align="right">
                    <template slot-scope="scope">
                        <span v-if="scope.row.commissionAmount||scope.row.commissionAmount==0">{{scope.row.commissionAmount.toQFW()}}</span>
                    </template>
                </el-table-column>
            </el-table>
        </el-dialog>
...
<script type="text/ecmascript-6">
    ...
    export default {
        ...
        computed:{
            ...
            settleAmount(){
                ...
                this.multipleSelection.forEach(row=>{
                    amount = amount.add(row.commissionAmount)
                })
                return amount
            }
        },
        ...
        methods:{
            ...
            //                     Code:data.referrerCode?data.referrerCode:data.refBranchCode,
            //                 })
            ...
            view(data){
                ...
                this.totalAmount = data.commissionAmount
                this.GetSerCommissionDetail({
                    ...
                    isCurrentMonth:this.formInline.isCurrentMonth?1:0,
                    TypeId:1,//1.服务商推荐奖励 2.服务商消费奖励
                    RefCode:data.referrerCode?data.referrerCode:data.refBranchCode,
                })
            },
            generate(){
