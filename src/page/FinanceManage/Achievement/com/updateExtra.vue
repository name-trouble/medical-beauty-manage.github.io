...
import {getBaseDataByCode,GetDropDownPermission,GetBaseBranchAccount,UpdateBranchOrderPayExt,
    ...
    methods: {
        copyData(data){
            ...
            this.ruleForm = {
                ...
                type: data.payTypeName+'|'+data.payType,
                ...
                topBranch:data.receiveBranchName+'|'+data.receiveBranchCode,
                ReceiveBranchCode:data.receiveBranchCode,
                ReceiveBranchName:data.receiveBranchName,
            }
            this.payDate = new Date(data.payDate.substring(0,4),data.payDate.substring(5,7)-1,data.payDate.substring(8,10))
        },
        //查找会员 下拉补全
        ...
        submitForm(formName) {
            ...
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    let data = JSON.parse(JSON.stringify(this.editMes))
                    for(var key in data){
                        if(data[key] == null){
                            data[key] = ''
                        }
                    }
                    data.branchCode = _this.ruleForm.BranchCode
                    data.payDate = _this.ruleForm.PayDate
                    data.payTypeName = _this.ruleForm.PayTypeName
                    data.payType = _this.ruleForm.PayType
                    data.memo = _this.ruleForm.Memo
                    data.receiveAccount = _this.ruleForm.ReceiveAccount
                    data.procFee = _this.ruleForm.ProcFee
                    data.receiveBranchCode = _this.ruleForm.ReceiveBranchCode
                    data.receiveBranchName = _this.ruleForm.ReceiveBranchName
                    data.realAmount = _this.ruleForm.RealAmount
                    data.modifiedBy = getCookie("userName")
                    data.modifiedUserId = getCookie("userId")
                    this.UpdateBranchOrderPayExt(data)
                }
            });
        },
        resetForm(formName) {
        ...
        async UpdateBranchOrderPayExt(params){
            let res = await UpdateBranchOrderPayExt(params)
            if(res.status){
