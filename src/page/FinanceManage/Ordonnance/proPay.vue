<template>
    <div>
        ...
        <el-table :data="tableData" border style="width: 100%;margin-top:20px" class="min_table4 smt"  v-loading="loading" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="orderProductCode" label="订单编号" min-width="100" align="center">
                 <template slot-scope="scope">
                    <el-button type="text" size="small" @click="views(scope.$index,tableData)">{{scope.row.orderProductCode}}</el-button>
                </template>
            </el-table-column>
            <el-table-column  prop="name" label="订单日期" min-width="100" align="center">
                <template slot-scope="scope">
            ...
            <el-table-column  prop="address"  label="操作" width="170" align="center">
                 <template slot-scope="scope">
                    <el-button type="primary" size="small" @click="pay(scope.$index,tableData)"  :disabled="scope.row.opStatus == 2">支付</el-button>
                    <el-button type="primary" size="small" @click="refund(scope.row)" :disabled="scope.row.opStatus == 2||!scope.row.isPayoff">退款</el-button>
        ...
        <!-- 支付弹框  -->
        <el-dialog title="支付" :visible.sync="editDialog" size="" top="5%">
                <edit @addClose="addClose" :mesCode="mesCode"  :id="mesId" :pay-mes-show="payMesShow" v-if="editDialog" style="width:1000px"></edit>
        </el-dialog>
        <!-- 支付弹框-->

        <!-- 退款弹窗 -->
        <el-dialog title="退款" :visible.sync="refundDialog" size="" top="5%">
                <refund @addClose="addClose" :mesCode="mesCode"  :id="mesId" :pay-mes-show="payMesShow" v-if="refundDialog" style="width:840px"></refund>
        </el-dialog>
        <!-- 退款 -->
...
<script>
    ...
    import refund from './com/refund'
    import CustomerView from "../ReportManage/com/customerDetail.vue"
    import {GetDropDownPermission,GetProofOrderProduct,DeleteProofOrderProduct,GetProofOrderProductDetail} from "@/api/myData"
import { getCookie } from '../../../config/mUtils';
    export default {
        ...
        methods: {
            ...
            refund(data){
                this.mesCode = data.orderProductCode
                this.mesId = data.id
            ...
            getlist(data,hospitalName,customerName){
                ...
                data.forEach(row=>{
                      html+=
                        ...
                        '<td width="30%" style="height: 30px;">'+row['productName']+'</td>'+
                        '<td width="30%" style="height: 30px;">'+row['unitName']+'</td>'+
                        '<td width="10%" style="height: 30px;">'+row['quantity']+'</td>'+
                        '<td width="10%" style="height: 30px;">'+row['realAmount']+'</td>'+
                        '<td width="20%" style="height: 30px;">'+row['usage']+'</td>'+
                        '</tr>'
                        sumData+=row.quantity.mul(row.realAmount)
                })
                let listHtml = '<table cellspacing="0" cellpadding="0" style="border-collapse:collapse;font-size: 14px;width:760px;margin: 0 auto;font-size:12px" bordercolor="#000000">'+
                    ...
                    '<tr style="height:30px;text-align:center">'+
                        '<td width="30%" style="border-bottom: 1px solid #111;">药品名称</td>'+
                        '<td width="30%" align="center" style="border-bottom: 1px solid #111;">规格</td>'+
                        '<td width="10%" style="border-bottom: 1px solid #111;">数量</td>'+
                        '<td width="10%" style="border-bottom: 1px solid #111;">单价</td>'+
