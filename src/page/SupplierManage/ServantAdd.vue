<template>
    <div class="addServant" style="margin:20px">

        <el-form :model="formData" ref="addForm" label-width="100px" class="demo-formData">
            <!--<el-form-item label="服务者编号" prop="Code" :rules="[ { required: true, message: '不能为空！', trigger: 'blur' }]">-->
            ...
            <el-form-item label="医院名称：" class='form_item_mt10' prop="hospitalCode" :rules="[ { required: true, message: '不能为空！', trigger: 'change' }]">
                <el-select v-model="formData.hospitalCode" @change="hosSelect" placeholder="请选择" style="width: 400px;">
                    <el-option v-for="(item,$index) in hospitals" :key="$index" value-key="Id" :label="item.SupplierName" :value="item.Code"></el-option>
                </el-select>
            </el-form-item>

            <!-- <el-form-item label="部门：" prop="partMent" :rules="[ { required: true, message: '不能为空！', trigger: 'change' }]">
                <el-select v-model="formData.partMent" @change="hosSelect" placeholder="请选择" style="width: 400px;">
                    <el-option v-for="(item,$index) in hospitals" :key="$index" value-key="Id" :label="item.SupplierName" :value="item.Code"></el-option>
                </el-select>
            </el-form-item> -->
            ...
            <el-form-item label="职务：" prop="jobs" class='form_item_mt10'>
                <el-tag :key="tag" v-for="tag in positionTags" :closable="true" type='primary' :close-transition="false" @close="handleCloseA(tag)">
                    {{tag}}
                </el-tag>
                <el-input style="width:100px;" class="input-new-tag" v-if="inputPostion" v-model="positionValue" ref="positionTags" size="small" @keyup.enter.native="handleInputConfirmA" @blur="handleInputConfirmA">
                </el-input>
            ...
            <el-form-item label="擅长：" prop="skillCode" class='form_item_mt10'>
                <el-tag :key="tag" v-for="tag in skillTags" :closable="true" type='primary' :close-transition="false" @close="handleCloseB(tag)">
                    {{tag}}
                </el-tag>
            ...
            <el-form-item label="国家/地区：" prop="country" :rules="[ { required: true, message: '不能为空！', trigger: 'blur' }]" class='form_item_mt10'>
                ...
                <!-- <el-input v-model="formData.country" style="width: 400px;"></el-input> -->
            </el-form-item>
            ...
            <el-form-item label="上架app：" prop="EnableStatus" required class='form_item_mt10'>
                <template>
                    <el-radio v-model="formData.EnableStatus" label="0">下架</el-radio>
                    <el-radio v-model="formData.EnableStatus" label="1">上架</el-radio>
                </template>
            </el-form-item>
            ...
            <el-form-item label="详细介绍：" class='form_item_mt10'  :show-message='true' style="width:600px" >
                <vue-editor height="200px" v-model="formData.richText" useCustomImageHandler @image-added="handleImageAdded"></vue-editor>
            </el-form-item>
            ...
            <el-form-item label="活动图：" class='form_item_mt10'>
            ...
            <el-form-item>
                <el-button type="primary" @click="submit" :loading="saveLoading">立即创建</el-button>
...
import { getSupplierBySupType, getBaseDataByCode, addServiceMan ,GetServiceManList,DelImg} from '@/api/myData'
export default {
    // name: "AddDoctor",
    // props: {
    //     data: {
    ...
    methods: {
        ...
        async addData() {
            /*this.formData.farePercent = (Number(this.formData.farePercent)).div(100)*/

            if(this.formData.serTypeCode !='004'){
                this.formData.ParentCode = ""
            }
            this.saveLoading = true
            let res = await addServiceMan(this.formData)
