        ...
        <el-dialog title="转赠" :visible.sync="dialogMove" size=''>
            <el-form :model="moveData" :rules="rules" style="width: 400px" ref="moveData" label-width="100px">
                <el-form-item label="转出账户：" class="form_item_mt10" prop="fromBranchCode">
                    <el-autocomplete v-model="moveData.fromBranch" :fetch-suggestions="queryCusFrom" placeholder="请输入内容" :trigger-on-focus="false" @select="selectCusIn" style="width:260px;margin-right:5px">
                ...
                <el-form-item label=" " style="margin:-10px 0 0 0" v-if="moveData.fromBranchCode.length>0">
                    <span>账户剩余医院积分：{{fromJF}}</span>, <span>预付款：{{fromYF}}</span>
                </el-form-item>
                <!--, <span>纹绣积分：{{fromWXJF}}</span>,<span>皮肤科积分：{{fromPFKJF}}</span>,<span>至尊VIP：{{fromVIPJF}}</span>-->
                <el-form-item label="转到账户：" class="form_item_mt10" prop="toBranchCode">
                    <el-autocomplete v-model="moveData.toBranch" :fetch-suggestions="queryCusTo" placeholder="请输入内容" :trigger-on-focus="false"  @select="selectCusTo" style="width:260px;margin-right:5px">
                        <template v-slot="{item}">
                            <my-item-transMember :item="item"></my-item-transMember>
                        </template>
                    </el-autocomplete>
