<template>
    <div class="rechargeList selCommon">
        ...
        <el-table :data="tableData" border style="width: 100%;margin-top:15px;margin-bottom:10px;" class="min_table smt" v-loading="loading">
            ...
            <el-table-column prop="balances" label="APP积分" min-width="80">
                <!-- <template slot-scope='scope'>
                    <el-button type="text" @click="viewhis(scope.row)" v-if="scope.row.balances!=0">{{scope.row.balances}}</el-button>
                    <span v-else @click="viewhis(scope.row)">0</span>
                </template> -->
            </el-table-column>
            ...
            <el-table-column prop="createOn" label="冻结状态" min-width="100">
                <template slot-scope="scope">
                    <el-tag type="success" v-if="!scope.row.isFrozen">
                        未冻结
                    </el-tag>
