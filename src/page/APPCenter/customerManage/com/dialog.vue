<template>
    <div class="selCommon">
        <el-form :inline="true" :model="formInline" class="demo-form-inline form_search" label-width="80px" >
            ...
            <el-form-item :label="label"  class="form_item_mt10" v-if="dialogMes.type == 6||dialogMes.type == 7">
        ...
        <div v-if="dialogMes.type==1">
            <el-table :data="tableData" border style="width: 100%;margin-top:15px;margin-bottom:10px;" max-height="500" class="min_table smt" v-loading="loading">
                ...
                <el-table-column prop="createOn" label="分享时间" min-width="100" align="center">
                    <template slot-scope="scope">
                        <span v-if="scope.row.createOn">{{scope.row.createOn}}</span>
                    </template>
                </el-table-column>
                <el-table-column prop="shareModule" label="分享模块" min-width="100"></el-table-column>
                <el-table-column prop="referId" label="ID" min-width="100"  align="center" ></el-table-column>
                <el-table-column prop="referName" label="名称" min-width="200">
                    <template slot-scope="scope">
                        <a target="_blank" class='shareUrl' :href="scope.row.shareUrl">{{scope.row.referName}}</a>
                    </template>
                </el-table-column>
                <el-table-column prop="referPrice" label="价格" min-width="100" align="right">
                    <template slot-scope="scope">
                        <span v-if="scope.row.referPrice">￥{{scope.row.referPrice.toQFW()}}</span>
                    </template>
                </el-table-column>
            </el-table>
        </div>
        <!-- 浏览数 -->
        <div v-if="dialogMes.type==2">
            <el-table :data="tableData" border style="width: 100%;margin-top:15px;margin-bottom:10px;" max-height="500" class="min_table smt" v-loading="loading">
                <el-table-column type="index" width="50" align="center"></el-table-column>
                <el-table-column prop="createOn" label="浏览时间" min-width="100" align="center">
                    <template slot-scope="scope">
                        <span v-if="scope.row.createOn">{{scope.row.createOn}}</span>
                    </template>
                </el-table-column>
                <el-table-column prop="createUserId" label="用户" min-width="100" align="center">
                     <template slot-scope="scope">
                        <span v-if="scope.row.createOn">{{JSON.parse(scope.row.userProfile).ip}}</span>
                    </template>
                </el-table-column>
                <el-table-column prop="shareModule" label="分享模块" min-width="100"></el-table-column>
                <el-table-column prop="invitedCode" label="邀请码" min-width="120" align="center"></el-table-column>
                <!-- <el-table-column prop="referName" label="名称" min-width="200" align="center"></el-table-column>
                <el-table-column prop="referPrice" label="价格" min-width="100"></el-table-column> -->
            </el-table>
        </div>
     ...
     <!-- 领取数 -->
        <div v-if="dialogMes.type==3">
         ...
         <!-- 登录数 -->
        <div v-if="dialogMes.type==4">
            <el-table :data="tableData" border style="width: 100%;margin-top:15px;margin-bottom:10px;" max-height="500" class="min_table smt" v-loading="loading">
                <el-table-column prop="loginTime" label="登录时间" min-width="100" align="center">
                    <template slot-scope="scope">
                ...
                <el-table-column prop="mobile" label="手机号" min-width="100" align="center"></el-table-column>
                <!-- <el-table-column prop="receiveBranchName" min-width="100" align="center"></el-table-column> -->
            </el-table>
        </div>
        <!-- 下单数 -->
        <div v-if="dialogMes.type==5">
            <el-table :data="tableData" border style="width: 100%;margin-top:15px;margin-bottom:10px;" max-height="500" class="min_table smt" v-loading="loading">
                <el-table-column prop="orderCode" label="订单编号" min-width="80" align="center">
                    <template slot-scope="scope">
                        <el-button type="text" @click="viewReport(scope.row)">{{scope.row.orderCode}}</el-button>
                    </template>
                </el-table-column>
                <el-table-column prop="orderTime" label="下单时间" min-width="100" align="center">
                    <template slot-scope="scope">
    ...
    export default {
        ...
        methods: {
            viewReport(data){
                ...
                this.$emit("viewReportCode",data.orderCode)
            },
