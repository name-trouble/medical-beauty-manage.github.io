<template>
    <div class="selCommon editRegister">
        ...
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="85px" class="demo-ruleForm">
            ...
            <el-form-item label="联系电话：" prop="phone" required class="form_item_mt0">
                <el-input v-model="ruleForm.phone"></el-input>
            </el-form-item>
            <el-form-item label="邮箱：" prop="email" class="form_item_mt0">
                <el-input v-model="ruleForm.email"></el-input>
            </el-form-item>
             <el-form-item label="会员类型：" prop="email" class="form_item_mt0">
            ...
            <el-form-item label="住址：" class="form_item_mt0">
                <el-select v-model="ruleForm.province" placeholder="请选择省" style="width: 120px">
                ...
                <el-input v-model="ruleForm.address" placeholder="详细地址" style="width: 100%"></el-input>
            </el-form-item>
            ...
            <el-form-item label="生日："  class="form_item_mt0">
             ...
             <el-form-item label="年龄：" prop="" class="form_item_mt0" >
                 ...
                 <span style="color:red" v-if="ruleForm.age>100">填写的年龄过大</span>
            </el-form-item>
            ...
            <el-form-item label="推荐人：" prop="referrerCode" class="form_item_mt0" >
                <!-- 平台不限制 其他权限在有推荐人的情况下不可修改 -->
                 <el-autocomplete v-model="ruleForm.recCode" :fetch-suggestions="queryCus" placeholder="请输入内容" :trigger-on-focus="false" @select="selectCus" style="width:183px;">
                     <!--<template v-slot="{item}">-->
                         <!--<my-item-memberItem :item="item"></my-item-memberItem>-->
                     <!--</template>-->
                 </el-autocomplete>
            </el-form-item>
            <el-form-item label="凭证：" class="form_item_mt0" >
                <el-button v-text="ruleForm.imageUrl?'查看凭证':'上传凭证'" type="primary" @click="uploadOpe"></el-button>
                <span style="color:red">修改推荐人可在此上传凭证（非必填）</span>
                <!-- {{ruleForm.imageUrl}} -->
            </el-form-item>
...
<script type="text/ecmascript-6">
    ...
    import {GetBaseDataAll,UpdateMembers,GetBranchByKeywords,GetTagByPage,GetLeastBranchCode,CheckPhoneNumber} from "@/api/myData"
    export default {
        props:{
            mes:{}
        },
        data () {
            ...
            var idCard = (rule, value, callback) => {
                ...
                if(!value){
                    return callback(new Error('请填写省份证号码'));
                }
            ...
            return {
                ...
                rules: {
                    name: [
                        ...
                        { min: 1, max: 100, message: '长度在 2 到 5 个字符', trigger: 'blur' }
                    ],
                    date1: [
        ...
        methods: {
            loadImg(val){
                this.ruleForm.imageUrl = val
            },
            uploadOpe(){

                this.$emit('uploadImg',this.ruleForm.imageUrl)
            },
            // 根据手机号码锁定推荐人禁止修改 没有推荐人可填写推荐人 (后改为不限制)
            phoneCha(val){
                if(val.length>=11){
                    this.checkRefer(val)
                }else{
...
//            添加会员 3普通会员 2认证会员
            async UpdateMembers(params){
                try{
                    this.saveLoading = true
                    let res = await UpdateMembers(params)
                    if(res.success){
...
//            获取基础信息
            ...
            submitForm(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        let obj = {
                            'Id':this.mes.Id,
                            'isAuth':this.mes.IsAuth,
                            'MemberName':this.ruleForm.name.removeSP(),
                            'code':this.mes.Code,
                            'registDate': this.mes.RegistDate,
                            ...
                            'sourcWayeCode':this.ruleForm.sourcWayeCode,
            ...
            copyWorth(data){
                ...
                if(data != ""){
                    ...
                    this.ruleForm.BranchTags = data.BranchTags?data.BranchTags:''
                    this.ruleForm.recCode = data.BranchTags? data.ReferrerName+'('+data.BranchTags+')':data.ReferrerName
