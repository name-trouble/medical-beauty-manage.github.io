    ...
    <div class="selCommon MemberManage">
        ...
        <el-table :data="tableData" border style="width: 100%;margin-top:15px;margin-bottom:10px" class="smt min_table"  v-loading="loading">
            ...
            <el-table-column prop="IsAuth" label="操作" min-width="120" align="center">
                <template slot-scope="scope">
                    <el-button @click="serviceObj(scope.row)" type="primary" size="small">查看服务店家</el-button>
                </template>
            </el-table-column>
        </el-table>
        <!-- 信息表 -->
        ...
        <el-dialog title="服务店家" :visible.sync="selectDialog" size="" :close-on-click-modal="false" top="10%" >
            <el-table :data="selectShopData" border style="width: 600px;margin-top:15px;margin-bottom:10px;" ref="multipleTable" class="min_table smt" max-height="600">
                ...
                <el-table-column prop="shopName" label="地址" min-width="120">
                    <template slot-scope="scope">
                        <span>{{scope.row.province+''+scope.row.city}}</span>
                    </template>
                </el-table-column>
                <el-table-column prop="shopTag" label="店家类型" min-width="80"></el-table-column>
            </el-table>
...
//            获取会员
            ...
            getPage(){
                this.selectShopData = this.shopData.slice((this.currentPage1-1)*this.size1,this.currentPage1*this.size1)
            },
